...
const API_KEY = "926386b8c8774f9d9671e5eeea926c77";
let savedArticles = [];

//News by source

intent("Run me news from $(source* (.*))", (p) => {
...
intent("What\'s up with $(term* (.*))", (p) => {
    let NEWS_API_URL = `https://newsapi.org/v2/everything?apiKey=${API_KEY}&country=us`
...
intent(`(show|what is|tell me|what's|what are|what're|read) (the|) (recent|latest|) $(N news|headlines)`,
  `(read|show|get|bring me|give me) (the|) (recent|latest) $(N news|headlines)`, (p) => {
    ...
    api.request(NEWS_API_URL,{headers: {"user-agent": 'user agent' }}, (error, response, body) => {
        ...
        if(!Object.keys(articles).length) {
            p.play('Sorry, please try that again.');
            return;
        }
